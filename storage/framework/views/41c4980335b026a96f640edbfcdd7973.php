...
<div class="bg-[#f8fafc] min-h-screen py-6 sm:py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
        <div class="mb-6 sm:mb-8 bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <a href="<?php echo e(route('admin.pengumuman.create')); ?>" class="inline-flex items-center justify-center gap-2 px-5 py-3 text-white font-semibold rounded-xl bg-[#063A76] hover:bg-[#052A57] transition-colors">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 sm:p-6 mb-6 sm:mb-8">
            <form action="<?php echo e(route('admin.pengumuman.index')); ?>" method="GET" class="flex flex-wrap gap-4 items-center">
                ...
                <?php if(request('search') || request('status')): ?>
                    <a href="<?php echo e(route('admin.pengumuman.index')); ?>" class="px-4 py-2.5 text-slate-700 rounded-xl bg-slate-100 hover:bg-slate-200 transition-colors">
                        Reset
                    </a>
                <?php endif; ?>
            </form>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-shadow">
                        <div class="flex items-start gap-6 p-6">
                            ...
                            <div class="flex items-center gap-2 flex-shrink-0">
                                <a href="<?php echo e(route('admin.pengumuman.edit', $item)); ?>" class="w-9 h-9 flex items-center justify-center text-white rounded-lg bg-[#063A76] hover:bg-[#052A57] transition-colors" title="Edit">
                                ...
                                <button type="button" class="w-9 h-9 flex items-center justify-center text-white rounded" style="background-color: #E83B2B;" onmouseover="this.style.backgroundColor='#D62B1C'" onmouseout="this.style.backgroundColor='#E83B2B'" title="Hapus"
                                    onclick="openDeleteModal('deletePengumumanModal', '<?php echo e(addslashes($item->title)); ?>', '/admin/pengumuman/<?php echo e($item->id); ?>')">
        ...
        <?php else: ?>
            <div class="bg-white rounded-2xl border border-slate-200 border-dashed p-12 text-center">
                ...
                <a href="<?php echo e(route('admin.pengumuman.create')); ?>" class="inline-flex items-center gap-2 px-6 py-3 text-white rounded-xl bg-[#063A76] hover:bg-[#052A57] transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                    Buat Pengumuman Baru
                </a>
            </div>
...
<!-- Delete Modal -->
<?php $__env->startComponent('components.delete-modal', ['id' => 'deletePengumumanModal', 'title' => 'Hapus Pengumuman?']); ?> <?php echo $__env->renderComponent(); ?>
