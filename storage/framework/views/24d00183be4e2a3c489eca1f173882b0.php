...
<div class="bg-[#f8fafc] min-h-screen py-6 sm:py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 sm:p-8">
            ...
            <form action="<?php echo e(route('admin.profile.update', $profileRuangan)); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                <?php echo csrf_field(); ?>
                <?php echo method_field('PUT'); ?>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Ruangan *</label>
                    <input type="text" name="room_name" required value="<?php echo e(old('room_name', $profileRuangan->room_name)); ?>" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#063A76] <?php $__errorArgs = ['room_name'];
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="floor" min="1" max="7" value="<?php echo e(old('floor', $profileRuangan->floor)); ?>" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#063A76]">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Kapasitas</label>
                        <input type="number" name="capacity" value="<?php echo e(old('capacity', $profileRuangan->capacity)); ?>" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#063A76]">
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Deskripsi</label>
                    <textarea name="description" rows="6" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#063A76]"><?php echo e(old('description', $profileRuangan->description)); ?></textarea>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Gambar Tersimpan</label>
                    <?php if($profileRuangan->images->count()): ?>
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                            <?php $__currentLoopData = $profileRuangan->images->sortBy('slot'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="border border-slate-200 rounded-lg p-2">
                                    <img src="<?php echo e(route('profile-ruangan.image', ['filename' => basename($image->image_path)])); ?>" alt="Gambar" class="w-full h-24 object-cover rounded mb-2">
                                    <button type="button" onclick="deleteStoredImage(<?php echo e($image->id); ?>, this)" class="w-full text-xs px-2 py-1 rounded bg-red-50 text-red-600 hover:bg-red-100">Hapus</button>
                                </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                    <?php else: ?>
...
unset($__errorArgs, $__bag); ?>
                ...
                <div>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" <?php echo e(old('is_active', $profileRuangan->is_active) ? 'checked' : ''); ?> class="w-4 h-4 text-[#063A76] rounded">
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['variant' => 'primary','size' => 'lg','type' => 'submit','icon' => 'check']); ?>Simpan Perubahan <?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginald0f1fd2689e4bb7060122a5b91fe8561)): ?>
<?php $attributes = $__attributesOriginald0f1fd2689e4bb7060122a5b91fe8561; ?>
<?php unset($__attributesOriginald0f1fd2689e4bb7060122a5b91fe8561); ?>
