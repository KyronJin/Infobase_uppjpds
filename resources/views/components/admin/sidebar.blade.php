<aside id="adminSidebar" class="fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-slate-200 shadow-sm transform -translate-x-full lg:translate-x-0 transition-transform duration-300">
    ...
    <nav class="px-4 py-4 space-y-1 overflow-y-auto h-[calc(100vh-80px)]">
        ...
        <a href="{{ route('admin.calendar.index') }}" class="admin-nav-link {{ request()->routeIs('admin.calendar.*') ? 'active' : '' }}">
            <i class="fas fa-calendar-days w-5"></i>
            <span>Calendar Aktifitas</span>
        </a>

        <a href="{{ route('admin.tata_tertib.index') }}" class="admin-nav-link {{ request()->routeIs('admin.tata_tertib.*') ? 'active' : '' }}">
        ...
        <a href="{{ route('admin.profil_pegawai.index') }}" class="admin-nav-link {{ request()->routeIs('admin.profil_pegawai.*') ? 'active' : '' }}">
            <i class="fas fa-id-card w-5"></i>
            <span>Profil Pegawai</span>
        </a>

        <a href="{{ route('admin.staff-of-month.index') }}" class="admin-nav-link {{ request()->routeIs('admin.staff-of-month.*') ? 'active' : '' }}">
